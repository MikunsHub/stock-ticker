from interfaces.constants import AlphaVantageResources


def mocked_response():
	return {
		AlphaVantageResources.TIME_SERIES_DAILY.name: {
			'Meta Data': {
				'1. Information': 'Daily Prices (open, high, low, close) and Volumes',
				'2. Symbol': 'AAPL',
				'3. Last Refreshed': '2024-06-18',
				'4. Output Size': 'Compact',
				'5. Time Zone': 'US/Eastern',
			},
			'Time Series (Daily)': {
				'2024-06-18': {
					'1. open': '217.5900',
					'2. high': '218.6300',
					'3. low': '213.0000',
					'4. close': '214.2900',
					'5. volume': '79943254',
				},
				'2024-06-17': {
					'1. open': '213.3700',
					'2. high': '218.9500',
					'3. low': '212.7200',
					'4. close': '216.6700',
					'5. volume': '93728300',
				},
				'2024-06-14': {
					'1. open': '213.8500',
					'2. high': '215.1700',
					'3. low': '211.3000',
					'4. close': '212.4900',
					'5. volume': '70122748',
				},
				'2024-06-13': {
					'1. open': '214.7400',
					'2. high': '216.7500',
					'3. low': '211.6000',
					'4. close': '214.2400',
					'5. volume': '97862729',
				},
				'2024-06-12': {
					'1. open': '207.3700',
					'2. high': '220.2000',
					'3. low': '206.9000',
					'4. close': '213.0700',
					'5. volume': '198134293',
				},
				'2024-06-11': {
					'1. open': '193.6500',
					'2. high': '207.1600',
					'3. low': '193.6300',
					'4. close': '207.1500',
					'5. volume': '172373296',
				},
				'2024-06-10': {
					'1. open': '196.9000',
					'2. high': '197.3000',
					'3. low': '192.1500',
					'4. close': '193.1200',
					'5. volume': '97262077',
				},
				'2024-06-07': {
					'1. open': '194.6500',
					'2. high': '196.9400',
					'3. low': '194.1400',
					'4. close': '196.8900',
					'5. volume': '53103912',
				},
				'2024-06-06': {
					'1. open': '195.6850',
					'2. high': '196.5000',
					'3. low': '194.1700',
					'4. close': '194.4800',
					'5. volume': '41181753',
				},
				'2024-06-05': {
					'1. open': '195.4000',
					'2. high': '196.9000',
					'3. low': '194.8700',
					'4. close': '195.8700',
					'5. volume': '54156785',
				},
				'2024-06-04': {
					'1. open': '194.6350',
					'2. high': '195.3200',
					'3. low': '193.0342',
					'4. close': '194.3500',
					'5. volume': '47471445',
				},
				'2024-06-03': {
					'1. open': '192.9000',
					'2. high': '194.9900',
					'3. low': '192.5200',
					'4. close': '194.0300',
					'5. volume': '50080539',
				},
				'2024-05-31': {
					'1. open': '191.4400',
					'2. high': '192.5700',
					'3. low': '189.9100',
					'4. close': '192.2500',
					'5. volume': '75158277',
				},
				'2024-05-30': {
					'1. open': '190.7600',
					'2. high': '192.1800',
					'3. low': '190.6300',
					'4. close': '191.2900',
					'5. volume': '49947941',
				},
				'2024-05-29': {
					'1. open': '189.6100',
					'2. high': '192.2470',
					'3. low': '189.5100',
					'4. close': '190.2900',
					'5. volume': '53068016',
				},
				'2024-05-28': {
					'1. open': '191.5100',
					'2. high': '193.0000',
					'3. low': '189.1000',
					'4. close': '189.9900',
					'5. volume': '52280051',
				},
				'2024-05-24': {
					'1. open': '188.8200',
					'2. high': '190.5800',
					'3. low': '188.0404',
					'4. close': '189.9800',
					'5. volume': '36326975',
				},
				'2024-05-23': {
					'1. open': '190.9800',
					'2. high': '191.0000',
					'3. low': '186.6250',
					'4. close': '186.8800',
					'5. volume': '51005924',
				},
				'2024-05-22': {
					'1. open': '192.2650',
					'2. high': '192.8231',
					'3. low': '190.2700',
					'4. close': '190.9000',
					'5. volume': '34648547',
				},
				'2024-05-21': {
					'1. open': '191.0900',
					'2. high': '192.7300',
					'3. low': '190.9201',
					'4. close': '192.3500',
					'5. volume': '42309401',
				},
				'2024-05-20': {
					'1. open': '189.3250',
					'2. high': '191.9199',
					'3. low': '189.0100',
					'4. close': '191.0400',
					'5. volume': '44361275',
				},
				'2024-05-17': {
					'1. open': '189.5100',
					'2. high': '190.8100',
					'3. low': '189.1800',
					'4. close': '189.8700',
					'5. volume': '41282925',
				},
				'2024-05-16': {
					'1. open': '190.4700',
					'2. high': '191.0950',
					'3. low': '189.6601',
					'4. close': '189.8400',
					'5. volume': '52845230',
				},
				'2024-05-15': {
					'1. open': '187.9100',
					'2. high': '190.6500',
					'3. low': '187.3700',
					'4. close': '189.7200',
					'5. volume': '70399988',
				},
				'2024-05-14': {
					'1. open': '187.5100',
					'2. high': '188.3000',
					'3. low': '186.2900',
					'4. close': '187.4300',
					'5. volume': '52393619',
				},
				'2024-05-13': {
					'1. open': '185.4350',
					'2. high': '187.1000',
					'3. low': '184.6200',
					'4. close': '186.2800',
					'5. volume': '72044809',
				},
				'2024-05-10': {
					'1. open': '184.9000',
					'2. high': '185.0900',
					'3. low': '182.1300',
					'4. close': '183.0500',
					'5. volume': '50759496',
				},
				'2024-05-09': {
					'1. open': '182.5600',
					'2. high': '184.6600',
					'3. low': '182.1100',
					'4. close': '184.5700',
					'5. volume': '48982972',
				},
				'2024-05-08': {
					'1. open': '182.8500',
					'2. high': '183.0700',
					'3. low': '181.4500',
					'4. close': '182.7400',
					'5. volume': '45057087',
				},
				'2024-05-07': {
					'1. open': '183.4500',
					'2. high': '184.9000',
					'3. low': '181.3200',
					'4. close': '182.4000',
					'5. volume': '77305771',
				},
				'2024-05-06': {
					'1. open': '182.3540',
					'2. high': '184.2000',
					'3. low': '180.4200',
					'4. close': '181.7100',
					'5. volume': '78569667',
				},
				'2024-05-03': {
					'1. open': '186.6450',
					'2. high': '187.0000',
					'3. low': '182.6600',
					'4. close': '183.3800',
					'5. volume': '163224109',
				},
				'2024-05-02': {
					'1. open': '172.5100',
					'2. high': '173.4150',
					'3. low': '170.8900',
					'4. close': '173.0300',
					'5. volume': '94214915',
				},
				'2024-05-01': {
					'1. open': '169.5800',
					'2. high': '172.7050',
					'3. low': '169.1100',
					'4. close': '169.3000',
					'5. volume': '50383147',
				},
				'2024-04-30': {
					'1. open': '173.3300',
					'2. high': '174.9900',
					'3. low': '170.0000',
					'4. close': '170.3300',
					'5. volume': '65934776',
				},
				'2024-04-29': {
					'1. open': '173.3700',
					'2. high': '176.0300',
					'3. low': '173.1000',
					'4. close': '173.5000',
					'5. volume': '68169419',
				},
				'2024-04-26': {
					'1. open': '169.8800',
					'2. high': '171.3400',
					'3. low': '169.1800',
					'4. close': '169.3000',
					'5. volume': '44838354',
				},
				'2024-04-25': {
					'1. open': '169.5250',
					'2. high': '170.6100',
					'3. low': '168.1511',
					'4. close': '169.8900',
					'5. volume': '50558329',
				},
				'2024-04-24': {
					'1. open': '166.5400',
					'2. high': '169.3000',
					'3. low': '166.2100',
					'4. close': '169.0200',
					'5. volume': '48251835',
				},
				'2024-04-23': {
					'1. open': '165.3500',
					'2. high': '167.0500',
					'3. low': '164.9200',
					'4. close': '166.9000',
					'5. volume': '49537761',
				},
				'2024-04-22': {
					'1. open': '165.5150',
					'2. high': '167.2600',
					'3. low': '164.7700',
					'4. close': '165.8400',
					'5. volume': '48116443',
				},
				'2024-04-19': {
					'1. open': '166.2100',
					'2. high': '166.4000',
					'3. low': '164.0750',
					'4. close': '165.0000',
					'5. volume': '68149377',
				},
				'2024-04-18': {
					'1. open': '168.0300',
					'2. high': '168.6400',
					'3. low': '166.5500',
					'4. close': '167.0400',
					'5. volume': '43122903',
				},
				'2024-04-17': {
					'1. open': '169.6100',
					'2. high': '170.6500',
					'3. low': '168.0000',
					'4. close': '168.0000',
					'5. volume': '50901210',
				},
				'2024-04-16': {
					'1. open': '171.7500',
					'2. high': '173.7600',
					'3. low': '168.2700',
					'4. close': '169.3800',
					'5. volume': '73711235',
				},
				'2024-04-15': {
					'1. open': '175.3600',
					'2. high': '176.6300',
					'3. low': '172.5000',
					'4. close': '172.6900',
					'5. volume': '73531773',
				},
				'2024-04-12': {
					'1. open': '174.2600',
					'2. high': '178.3600',
					'3. low': '174.2100',
					'4. close': '176.5500',
					'5. volume': '101670886',
				},
				'2024-04-11': {
					'1. open': '168.3400',
					'2. high': '175.4600',
					'3. low': '168.1600',
					'4. close': '175.0400',
					'5. volume': '91070275',
				},
				'2024-04-10': {
					'1. open': '168.8000',
					'2. high': '169.0900',
					'3. low': '167.1100',
					'4. close': '167.7800',
					'5. volume': '49709336',
				},
				'2024-04-09': {
					'1. open': '168.7000',
					'2. high': '170.0800',
					'3. low': '168.3500',
					'4. close': '169.6700',
					'5. volume': '42231444',
				},
				'2024-04-08': {
					'1. open': '169.0300',
					'2. high': '169.2000',
					'3. low': '168.2400',
					'4. close': '168.4500',
					'5. volume': '37216858',
				},
				'2024-04-05': {
					'1. open': '169.5900',
					'2. high': '170.3900',
					'3. low': '168.9500',
					'4. close': '169.5800',
					'5. volume': '41975776',
				},
				'2024-04-04': {
					'1. open': '170.2900',
					'2. high': '171.9200',
					'3. low': '168.8200',
					'4. close': '168.8200',
					'5. volume': '53355055',
				},
				'2024-04-03': {
					'1. open': '168.7900',
					'2. high': '170.6800',
					'3. low': '168.5800',
					'4. close': '169.6500',
					'5. volume': '45571129',
				},
				'2024-04-02': {
					'1. open': '169.0800',
					'2. high': '169.3400',
					'3. low': '168.2302',
					'4. close': '168.8400',
					'5. volume': '49013991',
				},
				'2024-04-01': {
					'1. open': '171.1900',
					'2. high': '171.2500',
					'3. low': '169.4750',
					'4. close': '170.0300',
					'5. volume': '43772506',
				},
				'2024-03-28': {
					'1. open': '171.7500',
					'2. high': '172.2300',
					'3. low': '170.5100',
					'4. close': '171.4800',
					'5. volume': '65672690',
				},
				'2024-03-27': {
					'1. open': '170.4100',
					'2. high': '173.6000',
					'3. low': '170.1100',
					'4. close': '173.3100',
					'5. volume': '60273265',
				},
				'2024-03-26': {
					'1. open': '170.0000',
					'2. high': '171.4200',
					'3. low': '169.5800',
					'4. close': '169.7100',
					'5. volume': '57388449',
				},
				'2024-03-25': {
					'1. open': '170.5650',
					'2. high': '171.9400',
					'3. low': '169.4500',
					'4. close': '170.8500',
					'5. volume': '54288328',
				},
				'2024-03-22': {
					'1. open': '171.7600',
					'2. high': '173.0500',
					'3. low': '170.0600',
					'4. close': '172.2800',
					'5. volume': '71160138',
				},
				'2024-03-21': {
					'1. open': '177.0500',
					'2. high': '177.4900',
					'3. low': '170.8400',
					'4. close': '171.3700',
					'5. volume': '106181270',
				},
				'2024-03-20': {
					'1. open': '175.7200',
					'2. high': '178.6700',
					'3. low': '175.0900',
					'4. close': '178.6700',
					'5. volume': '53423102',
				},
				'2024-03-19': {
					'1. open': '174.3400',
					'2. high': '176.6050',
					'3. low': '173.0300',
					'4. close': '176.0800',
					'5. volume': '55215244',
				},
				'2024-03-18': {
					'1. open': '175.5700',
					'2. high': '177.7100',
					'3. low': '173.5200',
					'4. close': '173.7200',
					'5. volume': '75604184',
				},
				'2024-03-15': {
					'1. open': '171.1700',
					'2. high': '172.6200',
					'3. low': '170.2850',
					'4. close': '172.6200',
					'5. volume': '121752699',
				},
				'2024-03-14': {
					'1. open': '172.9100',
					'2. high': '174.3078',
					'3. low': '172.0500',
					'4. close': '173.0000',
					'5. volume': '72571635',
				},
				'2024-03-13': {
					'1. open': '172.7700',
					'2. high': '173.1850',
					'3. low': '170.7600',
					'4. close': '171.1300',
					'5. volume': '51948951',
				},
				'2024-03-12': {
					'1. open': '173.1500',
					'2. high': '174.0300',
					'3. low': '171.0100',
					'4. close': '173.2300',
					'5. volume': '59544927',
				},
				'2024-03-11': {
					'1. open': '172.9400',
					'2. high': '174.3800',
					'3. low': '172.0500',
					'4. close': '172.7500',
					'5. volume': '58929918',
				},
				'2024-03-08': {
					'1. open': '169.0000',
					'2. high': '173.7000',
					'3. low': '168.9400',
					'4. close': '170.7300',
					'5. volume': '76267041',
				},
				'2024-03-07': {
					'1. open': '169.1500',
					'2. high': '170.7300',
					'3. low': '168.4900',
					'4. close': '169.0000',
					'5. volume': '71765061',
				},
				'2024-03-06': {
					'1. open': '171.0600',
					'2. high': '171.2400',
					'3. low': '168.6800',
					'4. close': '169.1200',
					'5. volume': '68587707',
				},
				'2024-03-05': {
					'1. open': '170.7600',
					'2. high': '172.0400',
					'3. low': '169.6200',
					'4. close': '170.1200',
					'5. volume': '95132355',
				},
				'2024-03-04': {
					'1. open': '176.1500',
					'2. high': '176.9000',
					'3. low': '173.7900',
					'4. close': '175.1000',
					'5. volume': '81510101',
				},
				'2024-03-01': {
					'1. open': '179.5500',
					'2. high': '180.5300',
					'3. low': '177.3800',
					'4. close': '179.6600',
					'5. volume': '73563082',
				},
				'2024-02-29': {
					'1. open': '181.2700',
					'2. high': '182.5700',
					'3. low': '179.5300',
					'4. close': '180.7500',
					'5. volume': '136682597',
				},
				'2024-02-28': {
					'1. open': '182.5100',
					'2. high': '183.1200',
					'3. low': '180.1300',
					'4. close': '181.4200',
					'5. volume': '48953939',
				},
				'2024-02-27': {
					'1. open': '181.1000',
					'2. high': '183.9225',
					'3. low': '179.5600',
					'4. close': '182.6300',
					'5. volume': '54318851',
				},
				'2024-02-26': {
					'1. open': '182.2400',
					'2. high': '182.7600',
					'3. low': '180.6500',
					'4. close': '181.1600',
					'5. volume': '40867421',
				},
				'2024-02-23': {
					'1. open': '185.0100',
					'2. high': '185.0400',
					'3. low': '182.2300',
					'4. close': '182.5200',
					'5. volume': '45119677',
				},
				'2024-02-22': {
					'1. open': '183.4800',
					'2. high': '184.9550',
					'3. low': '182.4600',
					'4. close': '184.3700',
					'5. volume': '52292208',
				},
				'2024-02-21': {
					'1. open': '181.9400',
					'2. high': '182.8888',
					'3. low': '180.6600',
					'4. close': '182.3200',
					'5. volume': '41529674',
				},
				'2024-02-20': {
					'1. open': '181.7900',
					'2. high': '182.4300',
					'3. low': '180.0000',
					'4. close': '181.5600',
					'5. volume': '53665553',
				},
				'2024-02-16': {
					'1. open': '183.4200',
					'2. high': '184.8500',
					'3. low': '181.6650',
					'4. close': '182.3100',
					'5. volume': '49752465',
				},
				'2024-02-15': {
					'1. open': '183.5500',
					'2. high': '184.4900',
					'3. low': '181.3500',
					'4. close': '183.8600',
					'5. volume': '65434496',
				},
				'2024-02-14': {
					'1. open': '185.3200',
					'2. high': '185.5300',
					'3. low': '182.4400',
					'4. close': '184.1500',
					'5. volume': '54630517',
				},
				'2024-02-13': {
					'1. open': '185.7700',
					'2. high': '186.2100',
					'3. low': '183.5128',
					'4. close': '185.0400',
					'5. volume': '56529529',
				},
				'2024-02-12': {
					'1. open': '188.4150',
					'2. high': '188.6700',
					'3. low': '186.7900',
					'4. close': '187.1500',
					'5. volume': '41781934',
				},
				'2024-02-09': {
					'1. open': '188.6500',
					'2. high': '189.9900',
					'3. low': '188.0000',
					'4. close': '188.8500',
					'5. volume': '45155216',
				},
				'2024-02-08': {
					'1. open': '189.3850',
					'2. high': '189.5350',
					'3. low': '187.3500',
					'4. close': '188.3200',
					'5. volume': '40962046',
				},
				'2024-02-07': {
					'1. open': '190.6400',
					'2. high': '191.0500',
					'3. low': '188.6100',
					'4. close': '189.4100',
					'5. volume': '53438955',
				},
				'2024-02-06': {
					'1. open': '186.8600',
					'2. high': '189.3100',
					'3. low': '186.7695',
					'4. close': '189.3000',
					'5. volume': '43490759',
				},
				'2024-02-05': {
					'1. open': '188.1500',
					'2. high': '189.2500',
					'3. low': '185.8400',
					'4. close': '187.6800',
					'5. volume': '69668820',
				},
				'2024-02-02': {
					'1. open': '179.8600',
					'2. high': '187.3300',
					'3. low': '179.2500',
					'4. close': '185.8500',
					'5. volume': '102551680',
				},
				'2024-02-01': {
					'1. open': '183.9850',
					'2. high': '186.9500',
					'3. low': '183.8200',
					'4. close': '186.8600',
					'5. volume': '64885408',
				},
				'2024-01-31': {
					'1. open': '187.0400',
					'2. high': '187.0950',
					'3. low': '184.3500',
					'4. close': '184.4000',
					'5. volume': '55467803',
				},
				'2024-01-30': {
					'1. open': '190.9400',
					'2. high': '191.8000',
					'3. low': '187.4700',
					'4. close': '188.0400',
					'5. volume': '55859370',
				},
				'2024-01-29': {
					'1. open': '192.0100',
					'2. high': '192.2000',
					'3. low': '189.5800',
					'4. close': '191.7300',
					'5. volume': '47145622',
				},
				'2024-01-26': {
					'1. open': '194.2700',
					'2. high': '194.7600',
					'3. low': '191.9400',
					'4. close': '192.4200',
					'5. volume': '44594011',
				},
			},
		},
		AlphaVantageResources.DIVIDENDS.name: {
			'symbol': 'AAPL',
			'data': [
				{
					'ex_dividend_date': '2024-05-10',
					'declaration_date': '2024-05-02',
					'record_date': '2024-05-13',
					'payment_date': '2024-05-16',
					'amount': '0.25',
				},
				{
					'ex_dividend_date': '2024-02-09',
					'declaration_date': '2024-02-01',
					'record_date': '2024-02-12',
					'payment_date': '2024-02-15',
					'amount': '0.24',
				},
				{
					'ex_dividend_date': '2023-11-10',
					'declaration_date': '2023-11-02',
					'record_date': '2023-11-13',
					'payment_date': '2023-11-16',
					'amount': '0.24',
				},
				{
					'ex_dividend_date': '2023-08-11',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.24',
				},
				{
					'ex_dividend_date': '2023-05-12',
					'declaration_date': '2023-05-04',
					'record_date': '2023-05-15',
					'payment_date': '2023-05-18',
					'amount': '0.24',
				},
				{
					'ex_dividend_date': '2023-02-10',
					'declaration_date': '2023-02-02',
					'record_date': '2023-02-13',
					'payment_date': '2023-02-16',
					'amount': '0.23',
				},
				{
					'ex_dividend_date': '2022-12-23',
					'declaration_date': '2022-12-19',
					'record_date': '2022-12-28',
					'payment_date': '2023-01-09',
					'amount': '0.0',
				},
				{
					'ex_dividend_date': '2022-11-04',
					'declaration_date': '2022-10-27',
					'record_date': '2022-11-07',
					'payment_date': '2022-11-10',
					'amount': '0.23',
				},
				{
					'ex_dividend_date': '2022-08-05',
					'declaration_date': '2022-07-28',
					'record_date': '2022-08-08',
					'payment_date': '2022-08-11',
					'amount': '0.23',
				},
				{
					'ex_dividend_date': '2022-05-06',
					'declaration_date': '2022-04-28',
					'record_date': '2022-05-09',
					'payment_date': '2022-05-12',
					'amount': '0.23',
				},
				{
					'ex_dividend_date': '2022-02-04',
					'declaration_date': '2022-01-27',
					'record_date': '2022-02-07',
					'payment_date': '2022-02-10',
					'amount': '0.22',
				},
				{
					'ex_dividend_date': '2021-11-05',
					'declaration_date': '2021-10-28',
					'record_date': '2021-11-08',
					'payment_date': '2021-11-11',
					'amount': '0.22',
				},
				{
					'ex_dividend_date': '2021-08-06',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.22',
				},
				{
					'ex_dividend_date': '2021-05-07',
					'declaration_date': '2021-04-28',
					'record_date': '2021-05-10',
					'payment_date': '2021-05-13',
					'amount': '0.22',
				},
				{
					'ex_dividend_date': '2021-02-05',
					'declaration_date': '2021-01-27',
					'record_date': '2021-02-08',
					'payment_date': '2021-02-11',
					'amount': '0.205',
				},
				{
					'ex_dividend_date': '2020-11-06',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.205',
				},
				{
					'ex_dividend_date': '2020-08-07',
					'declaration_date': '2020-07-30',
					'record_date': '2020-08-10',
					'payment_date': '2020-08-13',
					'amount': '0.82',
				},
				{
					'ex_dividend_date': '2020-05-08',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.82',
				},
				{
					'ex_dividend_date': '2020-02-07',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.77',
				},
				{
					'ex_dividend_date': '2019-11-07',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.77',
				},
				{
					'ex_dividend_date': '2019-08-09',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.77',
				},
				{
					'ex_dividend_date': '2019-05-10',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.77',
				},
				{
					'ex_dividend_date': '2019-02-08',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.73',
				},
				{
					'ex_dividend_date': '2018-11-08',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.73',
				},
				{
					'ex_dividend_date': '2018-08-10',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.73',
				},
				{
					'ex_dividend_date': '2018-05-11',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.73',
				},
				{
					'ex_dividend_date': '2018-02-09',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.63',
				},
				{
					'ex_dividend_date': '2017-11-10',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.63',
				},
				{
					'ex_dividend_date': '2017-08-10',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.63',
				},
				{
					'ex_dividend_date': '2017-05-11',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.63',
				},
				{
					'ex_dividend_date': '2017-02-09',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.57',
				},
				{
					'ex_dividend_date': '2016-11-03',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.57',
				},
				{
					'ex_dividend_date': '2016-08-04',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.57',
				},
				{
					'ex_dividend_date': '2016-05-05',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.57',
				},
				{
					'ex_dividend_date': '2016-02-04',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.52',
				},
				{
					'ex_dividend_date': '2015-11-05',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.52',
				},
				{
					'ex_dividend_date': '2015-08-06',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.52',
				},
				{
					'ex_dividend_date': '2015-05-07',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.52',
				},
				{
					'ex_dividend_date': '2015-02-05',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.47',
				},
				{
					'ex_dividend_date': '2014-11-06',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.47',
				},
				{
					'ex_dividend_date': '2014-08-07',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '0.47',
				},
				{
					'ex_dividend_date': '2014-05-08',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '3.29',
				},
				{
					'ex_dividend_date': '2014-02-06',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '3.05',
				},
				{
					'ex_dividend_date': '2013-11-06',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '3.05',
				},
				{
					'ex_dividend_date': '2013-08-08',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '3.05',
				},
				{
					'ex_dividend_date': '2013-05-09',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '3.05',
				},
				{
					'ex_dividend_date': '2013-02-07',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '2.65',
				},
				{
					'ex_dividend_date': '2012-11-07',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '2.65',
				},
				{
					'ex_dividend_date': '2012-08-09',
					'declaration_date': 'None',
					'record_date': 'None',
					'payment_date': 'None',
					'amount': '2.65',
				},
			],
		},
		AlphaVantageResources.SPLITS.name: {
			'symbol': 'AAPL',
			'data': [
				{'effective_date': '2020-08-31', 'split_factor': '4.0000'},
				{'effective_date': '2014-06-09', 'split_factor': '7.0000'},
				{'effective_date': '2005-02-28', 'split_factor': '2.0000'},
				{'effective_date': '2000-06-21', 'split_factor': '2.0000'},
			],
		},
	}
